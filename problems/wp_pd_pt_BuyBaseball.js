...
function draw(){
        ...
        let item1=baseballVar("random");
        let stateTax = Math.floor((Math.random()*1+6.5)*10)/10;
        let item1Name=item1.item;
        let item1Price=item1.cost;
        let percentDiscount=Math.ceil(Math.random()*4)*10
        let nam=names();
        let cost = rounding(item1Price*(1-percentDiscount/100)*(1+stateTax/100),"hundredth");
        
        //rounding(item1Price*(1+stateTax/100),"hundredth");
        
        let question=nam+" went to the sporting goods store and purchased a "+item1Name+", which cost $"+item1Price+".  If there was a "+percentDiscount+"% discount and the state sales tax was "+stateTax+"%,  how much did "+nam+" pay for the "+item1Name+"?";
        let answer=nam+" paid $"+cost+" for the "+item1Name+".";
