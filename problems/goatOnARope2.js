...
function draw(){
        ...
        let scale = .2;
        let measur="ft.";
        let placeValue="tenth";
        let figX=100;
        let figY=130;
        let recWid=Math.random()*50+80; 
        let recHei=Math.random()*50+80;
        let recWidDim=Math.floor(recWid*scale)      
        let recHeiDim=Math.floor(recHei*scale);
        let triH=Math.random()*60+40;
        let triW=Math.random()*60+40;
        let dia=Math.sqrt(Math.pow(triH,2)+Math.pow(triW,2));    
        let ropeLen=dia;
        let ropeLenDim=Math.floor(ropeLen*scale);
        let ropeX1=figX+recWid;
        let ropeY1=figY;
        let ropeX2=ropeX1+triW;
        let ropeY2=ropeY1-triH;

        let area; 

        if(ropeLenDim<=recWidDim&&ropeLenDim<=recHeiDim){
            area = rounding(Math.pow(ropeLenDim,2)*Math.PI*.75,"tenth");            
        }
        if(ropeLenDim<=recWidDim&&ropeLenDim>recHeiDim){            
            let diff=ropeLenDim-recHeiDim;
            area = rounding(Math.pow(ropeLenDim,2)*Math.PI*.75+Math.pow(diff,2)*Math.PI*.25,"tenth"); 
        }
       
        if(ropeLenDim>recWidDim&&ropeLenDim<=recHeiDim){          
            let diff=ropeLenDim-recWidDim;
            area = rounding(Math.pow(ropeLenDim,2)*Math.PI*.75+Math.pow(diff,2)*Math.PI*.25,"tenth");             
        }
        if(ropeLenDim>recWidDim&&ropeLenDim>recHeiDim){            
            var diff1=ropeLenDim-recHeiDim
            var diff2=ropeLenDim-recWidDim
            area = rounding(Math.pow(ropeLenDim,2)*Math.PI*.75+Math.pow(diff1,2)*Math.PI*.25+Math.pow(diff2,2)*Math.PI*.25,"tenth"); 
        }
        ...
        let ques = "If a goat is attached to a rope at the corner of the below building, how much area is available for the goat to eat? Round you answers to the tenth place."; 
        //**********WRITE ANSWER********** - 5 OF 7 
        let ans = "The area where the goat can feed is "+area+" square feet."
...
//GEOMETRIC FIGURES
        ...
        drawRect(ctx,figX,figY,recWid,recHei,"white");

        //type is vert "top/bottom" or hori "left/right"
        //drawCircle(ctx,x,y,rad,s,f,clr)
        drawCircle(ctx,ropeX2,ropeY2,5,0,2*Math.PI,"red");
...
//DIMENSIONAL LINES
        ...
        //drawArrowVertRight(ctx,figX+rectWid,figY,rectHei,rectHeiDim,meas,20);

        //drawArrowDiag(ctx,x,y,x2,y2,text,meas,offSet)
        drawArrowDiag(ctx,ropeX1,ropeY1,ropeX2,ropeY2,ropeLenDim,measur,-80);

        //drawArrowHoriTop(x,y,l,text,meas,offSet)
        //drawArrowHoriTop(ctx,figX,figY,width1,width1Dim,"ft.",50);

        //drawArrowHoriBottom(ctx,x,y,l,text,meas,offSet)
        drawArrowHoriBottom(ctx,figX,figY+recHei,recWid,recWidDim,measur,20);

        //drawArrowVertLeft(ctx,x,y,l,text,meas,offSet)
        drawArrowVertLeft(ctx,figX,figY,recHei,recHeiDim,measur,-20);
...
//DIMENSIONAL LINES FUNCTIONS      
        ...
        function drawArrowDiag(ctx,x,y,x2,y2,text,meas,offSet){
            ...
            ctx.lineWidth=1;
            ctx.fillStyle="black";  
            ctx.fillText(text+" "+meas,x+x2*.4+offSet,y+y2*.05);   
        }       
        ...
        function drawArrowVertLeft(ctx,x,y,l,text,meas,offSet){
            ...
            ctx.lineWidth=1;
            ctx.fillStyle="black";
            ctx.fillText(text+" "+meas,x+offSet-35,y+l*.5);
        }
//ADD NAMES - SHUFFLE INFO - GET RANDOM INT FUNCTIONS
        function names(){
