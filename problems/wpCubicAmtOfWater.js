...
function draw(){
        ...
        let amtPerMinute=Math.ceil(Math.random()*30+1)/10;
        let time=Math.ceil(Math.random()*8);
        let time2=Math.ceil(Math.random()*8);
        ...
        if(randomPlace==3){
            placeValue="hundredth"
        };
        let answer=rounding(time2*60/time*amtPerMinute,placeValue);

        
       
              
        //**********WRITE QUESTION - 4 OF 7**********
        let ques = "A pipe discharges "+amtPerMinute+" cubic feet of water in "+time+" minutes.  How many cubic feet will it discharge in "+time2+" hours?  Round to the "+placeValue+" place."; 
        //**********WRITE ANSWER********** - 5 OF 7 
        let ans = "The pipe will discharge "+answer+" cubic feet of water in 8 hours."
...
//ADD NAMES - SHUFFLE INFO - GET RANDOM INT FUNCTIONS
        ...
        //decimalToFraction(c) c is mixed number;
        //decimalToFraction(c);
...
//ADD NAMES - SHUFFLE INFO - GET RANDOM INT FUNCTIONS
    ...
    if(place=="ten thousandth"){
        ...
        function decimalToFraction(c){
            let answer;
            let whole=Math.floor(c);
            let numerator1=rounding(c-whole,"hundredth").toString();
            let numeratorLength=Number(numerator1.length);
            let denominator;
            if(numeratorLength==3){
                numerator1=numerator1*10;
                denominator=10;
            }else 
            if(numeratorLength==4){
                numerator1=numerator1*100;
                denominator=100;
            }else 
            if(numeratorLength==5){
                numerator1=numerator1*1000;
                denominator=1000
            }  
            reduce(numerator1,denominator)    
            function reduce(numerator,denominator){
                var gcd = function gcd(a,b){
                  return b ? gcd(b, a%b) : a;
                };
                gcd = gcd(numerator,denominator);
                answer = [whole+" "+numerator/gcd+"/"+denominator/gcd].toString();
            }
            return answer
        }
