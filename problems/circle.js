...
function draw(){
        ...
        let figX=100;
        let figY=90;
        let radius1=Math.round(Math.random()*20+50);
        let diameter1=radius1*2;
        let radius1Dim=Math.floor(radius1*scale);
        let diameter1Dim=Math.round(diameter1*scale);
        let perimeter = rounding(diameter1Dim*Math.PI,"tenth");
        let area = rounding(Math.pow(radius1Dim,2)*Math.PI,"tenth"); 
...
//DIMENSIONAL LINES 
        ...
        //drawArrowDiag(ctx,figX,figY,figX+rectWid,figY+rectHei,rectDiagDim,meas,5);

        //drawArrowHoriTop(x,y,l,text,meas,offSet)
        drawArrowHoriTop(ctx,figX-radius1,figY,diameter1,diameter1Dim,measur,0);

        //drawArrowHoriBottom(ctx,x,y,l,text,meas,offSet)
        drawArrowHoriBottom(ctx,figX,figY,radius1,radius1Dim,measur,20);

         //drawArrowVertLeft(ctx,x,y,l,text,meas,offSet)
        drawArrowVertLeft(ctx,figX,figY-radius1,diameter1,diameter1Dim,measur,0);
        drawArrowVertLeft(ctx,figX,figY-radius1,diameter1,diameter1Dim,measur,130);  
        
//ADD NAMES - SHUFFLE INFO - GET RANDOM INT
...
//DIMENSIONAL LINES 
        ...
        function drawArrowVertLeft(ctx,x,y,l,text,meas,offSet){
            ...
            ctx.lineWidth=1;
            ctx.fillStyle="black";
            ctx.fillText(text+" "+meas,x+offSet-40,y+l*.5);
        }
        //ADD NAMES - SHUFFLE INFO - GET RANDOM INT
// ten thousands, thousands, hundreds, tens, ones, tenth, hundredth, thousandth, ten thousandth
