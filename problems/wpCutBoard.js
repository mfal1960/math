...
function draw(){
        ...
        let name=names();
        let BoardLengthRan=Math.ceil(Math.random()*3);
        let boardLength;
        if(BoardLengthRan==1){
            boardLength=4;
        }else
        ...
        let cutFeet=Math.ceil(Math.random()*3);
       
        let cutInches=createMixNumbers();
        let cutInches2=mixFracToDec(cutInches);
       
        let cutBoard=cutFeet+" feet "+cutInches+" inches ";     
        let boardLengthIn=boardLength*12;
             
        let bladeCut=1/8;
        let bladeCutString="1/8";
      
        let amtToRemoveInch=cutFeet*12+cutInches2+bladeCut;
        let amtBoardLeftIn=boardLengthIn-amtToRemoveInch;
        let finalFeetRemaining=Math.floor(amtBoardLeftIn/12);
        let inchAsFeetRemaining=Math.round((amtBoardLeftIn/12-Math.floor(amtBoardLeftIn/12))*12*1000)/1000;
        let finalInchesRemaining=Math.floor(inchAsFeetRemaining);
        let finalfracInch=decimalToFraction(inchAsFeetRemaining-finalInchesRemaining);
        
        let answer;
        if(finalFeetRemaining==0){
            answer=finalInchesRemaining+" "+finalfracInch+" inches";
        }else 
        if(finalFeetRemaining==1){
            answer=finalFeetRemaining+" foot "+finalInchesRemaining+" "+finalfracInch+" inches";
        }else 
        if(finalFeetRemaining>1){
            answer=finalFeetRemaining+" feet "+finalInchesRemaining+" "+finalfracInch+" inches";
        }
        
        //**********WRITE QUESTION - 3 OF 4**********
        let ques = "If "+name+" has to cut a board that is "+cutBoard+" long from an "+boardLength+" foot board, how much of the board will be left.  The blade takes away an additional "+bladeCutString+" of an inch.  Express your answer as a mixed number."; 
        //**********WRITE ANSWER********** - 4 OF 4 
        let ans = "There will be "+answer+" left."
